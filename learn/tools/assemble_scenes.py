...
def build_html(manifest: dict[str, Any], scenes_dir_name: str) -> tuple[str, dict[str, Any]]:
    ...
    for i in range(1, len(tiled)):
        ...
        seam_lines.append(
            ...
            f'duration: 0.34, ease: "power3.in" }}, {exit_at}); // anchor-exempt: seam mechanics stamped from an anchored scene boundary'
        )
        seam_lines.append(f'      tl.set("#slot-{prev["id"]}", {{ autoAlpha: 0 }}, {cut}); // anchor-exempt: seam mechanics stamped from an anchored scene boundary')
        seam_lines.append(
            f'      tl.fromTo("#slot-{cur["id"]}", {{ {prop}: {-10 * direction}, autoAlpha: 0.35 }}, '
            f'{{ {prop}: 0, autoAlpha: 1, duration: 0.42, ease: "power4.out", immediateRender: false }}, {cut}); // anchor-exempt: seam mechanics stamped from an anchored scene boundary'
        )
    ...
    if intro_fade > 0:
        ...
        bookend_lines.append(
            f'      tl.to("#fade-in", {{ autoAlpha: 0, duration: {intro_fade}, ease: "power2.out" }}, 0); // anchor-exempt: bookend fade, relative to the composition boundary'
        )
    if outro_start is not None:
        bookend_lines.append(
            f'      tl.fromTo("#fade-out", {{ autoAlpha: 0 }}, '
            f'{{ autoAlpha: 1, duration: {outro_fade}, ease: "power2.in", immediateRender: false }}, {outro_start}); // anchor-exempt: bookend fade, relative to the composition boundary'
        )
